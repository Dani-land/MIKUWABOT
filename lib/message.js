...
const canalSeleccionado = canales[Math.floor(Math.random() * canales.length)]
    ...
    const dynamicButtons = buttons.map((btn) => ({
      ...
      contextInfo: {
        ...
        externalAdReply: {
          title: botname,
          body: botname2 + ', Built With ✿ Danielrxz',
          mediaType: 1,
  ...
  const pfp = botSetting.icon || ''
    ...
    const canalNam = botSetting.nameid || 'ʜᴀᴛsᴜɴᴇ ᴍɪᴋᴜ ʙᴏᴛ'
      ...
      jid,
      {
        extendedTextMessage: {
          ...
          contextInfo: {
            ...
            externalAdReply: {
              title: namebugsito,
              body: `${namebugg2}, Built With ♡ MikuBot`,
              mediaType: 1,
              renderLargerThumbnail: false,
              previewType: `PHOTO`,
  ...
  client.sendContextInfoIndex = async (
    ...
    const namebot2 = config.body || `${settings.namebot2}, Built With ♡ By MikuBot`
    const canalId = settings.id ||'120363420575743790@newsletter'
    const canalName = settings.nameid || 'ʜᴀᴛsᴜɴᴇ ᴍɪᴋᴜ ʙᴏᴛ'
    const sourceUrl =
