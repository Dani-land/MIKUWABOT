...
import { logNewSubbotToChannel } from './channelLogger.js'
...
globalThis.db.data.settings[botDir].type = 'Sub'
      ...
      if (isNewLogin && client) {
        try {
          const ownerBotId = client.user.id.split(':')[0] + '@s.whatsapp.net'
          await logNewSubbotToChannel({
            client,
            botId: ownerBotId,
            user: senderId,
            pushname: m?.pushName,
            subbotNumber: sock.userId,
          })
        } catch (e) {
          console.log(chalk.gray(`[ ✿  ] No se pudo avisar al canal del nuevo subbot: ${e.message}`))
        }
      }
