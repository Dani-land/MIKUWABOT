export default {
  ...
  run: async ({client, m, args}) => {
    ...
    if (args[0].toLowerCase() === 'all') {
      ...
      user.coins = 0
      user.bank += count
      await m.reply(`✅ Has depositado *¥${count.toLocaleString()} ${monedas}* en tu Banco, ahora esta seguro 🔐`)
      return true
    }

    if (!Number(args[0]) || parseInt(args[0]) < 1) {
      return m.reply('ꕥ Ingresa una cantidad *válida* para depositar')
    }

    const count = parseInt(args[0])
    if (user.coins <= 0 || user.coins < count) {
      return m.reply('❀ No tienes suficientes *${monedas}* fuera del banco para depositar')
    }
