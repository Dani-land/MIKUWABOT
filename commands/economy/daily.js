...
user.coins = user.coins ?? 0
  ...
  return m.reply(
    `「✿ Recompensa diaria reclamada, ganaste: *${recompensa.toLocaleString()} ${monedas}*! (Día *1*)\n` +
    `> Día *2* » *¥${siguiente.toLocaleString()}*` +
    (perdioRacha ? `\n> ꕥ Perdiste tu racha de días usando #daily` : '')
  )
}
    ...
    const rachaExtra = user.dailyStreak >= 10
      ? `\n> ✰ ¡Racha de *${user.dailyStreak}* días`
      : ''

    await m.reply(
      `「✿ Recompensa diaria reclamada, ganaste: *¥${recompensa.toLocaleString()} ${monedas}* (Día *${user.dailyStreak}*)\n` +
